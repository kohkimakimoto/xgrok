...
import (
	"github.com/kohkimakimoto/xgrok/xgrok/client/mvc"
...
func applyUpdate(s mvc.State, result *check.Result) {
	...
	log.Error("Error while updating xgrok: %v", err)
	if errRecover != nil {
		log.Error("Error while recovering from failed xgrok update, your binary may be missing: %v", errRecover.Error())
	}
